--  get only STATUS changes for ticket
-- issuenum
-- pkey
...
WHERE cg.issueid=(select id from jiraissue where issuenum = 115 and project in (select id from project where pkey = 'PROJECT_KEY'))
order by 1,3,4;