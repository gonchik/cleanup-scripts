/*
    ...
    Remove only duplicated records older than 1 month.
...
WITH sorted_history AS (SELECT hist."ID",
                               ...
                               hist."CREATED",
                               LEAD(hist."NEW_VALUE")
                                   OVER (PARTITION BY hist."OBJECT_ID", hist."AFFECTED_ATTRIBUTE" ORDER BY hist."CREATED") AS next_new_value,
                                LEAD(hist."OLD_VALUE")
                                    OVER (PARTITION BY hist."OBJECT_ID", hist."AFFECTED_ATTRIBUTE" ORDER BY hist."CREATED") AS next_old_value
...
WHERE sh."OLD_VALUE" = sh.next_new_value
  ...
  and sh."NEW_VALUE" != ''),
    matches_3 AS (SELECT sh.*,
    'old = new' as "Type"
FROM sorted_history sh
WHERE sh."OLD_VALUE" = sh."NEW_VALUE"
  and sh."NEW_VALUE" != ''),
    all_match AS (select *
FROM matches
union
...
GROUP BY "OBJECT_ID",
    ...
    "Type"
HAVING COUNT(*) > 0),
    ranked_matches AS (select m.*,
    ROW_NUMBER()
    OVER (PARTITION BY m."OBJECT_ID", m."AFFECTED_ATTRIBUTE", m."Type" ORDER BY m."CREATED" DESC) AS rn
FROM all_match m
    JOIN
    counts c ON m."OBJECT_ID" = c."OBJECT_ID")
DELETE
FROM public."AO_8542F1_IFJ_OBJ_HIST"
